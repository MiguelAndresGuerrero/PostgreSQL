...
select string_agg(substring('0123456789abcdefghijklmnopqrstuvwxyz', round(random() * 30)::integer, 1), '') from generate_series(1, $1);
$$ language sql;

select string_agg(substring('0123456789abcdefghijklmnopqrstuvwxyz', round(random() * 30)::integer, 1), '') as NewPassword 
from generate_series(7, 10, 1);
...
drop function generate_ramdon_password1;
...
-- retorna un text pero de una variable

drop function generate_ramdon_password4;

create function generate_ramdon_password4(long INTEGER)
returns TEXT as $$
declare
	miVariable text;
BEGIN
	miVariable := (select string_agg(substring('0123456789abcdefghijklmnopqrstuvwxyz$.!', round(random() * length('0123456789abcdefghijklmnopqrstuvwxyz$.!'))::integer, 1), '') as NewPassword 
	from generate_series(0, long));
...
select generate_ramdon_password4(4);

select sha512(password::bytea), username from credenciales;

update credenciales set password = sha512(generate_ramdon_password1(12) where username = 'brush99');
...
select * from credenciales where sha512('w5dk76cpnn$4'::bytea) = password_sha and username = 'brush99';
...
SELECT * FROM usuarios ORDER BY seguidores DESC LIMIT 2;

INSERT INTO usuarios (nombre, apellido, email, ip, lugar, plataforma, usuario, siguiendo, seguidores) VALUES
